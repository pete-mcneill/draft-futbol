...
class SubBar extends ConsumerWidget {
  ...
  const SubBar({super.key, required this.team});
            ...
            Row(
              ...
              children: [
                if (ref.watch(subsControllerProvider).subsModeActive &&
                    ref.watch(subsControllerProvider).subsToSave) ...[
                  ElevatedButton(
                    onPressed: () async {
                      ref
                          .read(subsControllerProvider.notifier)
                          .enableSubsInprogress();
                      ...
                      await ref
                          ...
                          .getDraftData(leagueIds);
                      final test = ref.read(draftDataControllerProvider);
                      ref
                          .read(subsControllerProvider.notifier)
                          .disableSubsInprogress();
                      ref
                          .read(subsControllerProvider.notifier)
                          .disableSubsMode(team.id!);
                    },
                ...
                if (ref
                    .watch(subsControllerProvider.notifier)
                    .checkForSubsForTeam(team))
                  ElevatedButton(
                    onPressed: () async {
                      ref
                          .read(subsControllerProvider.notifier)
            ...
            if (ref.watch(subsControllerProvider).subsModeActive)
              Row(
                children: [
                  Flexible(
                      child: Text(
                    "To make a sub, drag a player from the bench to a valid starting position",
